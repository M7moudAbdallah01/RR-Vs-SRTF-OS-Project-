...
// تأكد من عمل import للـ Models لو هي في package تانية
...
lbl.setOpaque(true); 
lbl.setBackground(new Color(0, 102, 204)); // لون أزرق زي الصورة
lbl.setForeground(Color.WHITE); // نص أبيض
lbl.setBorder(BorderFactory.createEmptyBorder(5, 10, 5, 10)); // مسافات داخلية
lbl.setAlignmentX(Component.CENTER_ALIGNMENT); // يخليه في النص

p.add(lbl);
p.add(Box.createVerticalStrut(5)); // بيسيب مسافة صغيرة بين كل Process والتانية
...
private void updateComparisonSummary() {
    // تحديث الأرقام في الـ Labels اللي إنت عملتها في الـ Design
    // تأكد من أسماء الـ Labels (Variable Names) عندك في الـ NetBeans
    ...
    // تحديد الفائز النهائي بناءً على أقل وقت انتظار (WT)
...
class GanttPanel extends JPanel {
    ...
    public GanttPanel(List<GanttRecord> records) {
        ...
        this.setPreferredSize(new Dimension(500, 80)); // مقاس المستطيل
...
@Override
    protected void paintComponent(Graphics g) {
        ...
        int x = 20; // نقطة البداية
        int y = 20; // الارتفاع من فوق
        int height = 30; // طول مستطيل العملية
        int scale = 15;  // العرض لكل ثانية زمنية

        for (GanttRecord r : records) {
            int width = (r.endTime - r.startTime) * scale;
            
            // رسم المستطيل (بدون ألوان، فقط إطار أسود)
            g.drawRect(x, y, width, height);
            
            // كتابة اسم العملية (مثلاً P1) في النص
            g.drawString("P" + r.processId, x + (width/4), y + 20);
            
            // كتابة التوقيت تحت المستطيل
            g.drawString(String.valueOf(r.startTime), x, y + height + 15);
            
            x += width;
            
            // رسم التوقيت الأخير بعد انتهاء آخر عملية
...
private String generateGanttString(List<GanttRecord> gantt) {
    ...
    for (int i = 0; i < gantt.size(); i++) {
        ...
        // دمج العمليات المتكررة ورا بعض (عشان الـ SRTF ميبقاش 0-1-2-3 ورا بعض)
...
private String generateReadyQueueString(List<Process> processes) {
    ...
    for (int i = 0; i < processes.size(); i++) {
        ...
        if (i < processes.size() - 1) {
            sb.append(" → "); // إضافة سهم بين العمليات
        }
    }
    sb.append(" ]");
    return sb.toString();
